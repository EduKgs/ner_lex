...
def batch_predict(raw_text, model, device, args, id2query):
    ...
    with torch.no_grad():
        ...
        token_words = torch.from_numpy(np.array(cut_word(raw_text[0],tokenizer))).long().unsqueeze(0).to(device)
...
if __name__ == "__main__":
    args_path = "checkpoints/bert_bilstm_crf_cner/args.json"
